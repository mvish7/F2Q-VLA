...
def build_param_groups(model: Any, config: Any) -> list[dict]:
    ...
    # Module names that are part of the LLM interface and should share LLM LR
    llm_interface_modules = {"embed_tokens", "lm_head"}
    
    groups = {"default": [], "llm": [], "vision_lora": []}
    ...
    for name, param in model.named_parameters():
        ...
        if "lora_" in name and "vision_tower" in name:
            ...
            groups["llm"].append(param)
        elif any(mod in name for mod in llm_interface_modules):
            groups["llm"].append(param)
        else:
    ...
    if groups["llm"]:
        param_groups.append({"params": groups["llm"], "lr": llm_lora_lr})
    ...
    print(f"  LLM (LR={llm_lora_lr}): {sum(p.numel() for p in groups['llm']):,} params")
